...
(function () {
  ...
  function f32() {
    document.addEventListener("pointerdown", e => {
      ...
      b.classList.add("btn-squish");
    });
    const rel = () => document.querySelectorAll(".btn-squish").forEach(b => {
      b.classList.remove("btn-squish");
      b.classList.add("btn-spring");
      setTimeout(() => b.classList.remove("btn-spring"), 400);
    });
  ...
  // 33. SCROLL AURORA
  // Purely decorative (hue-shifts the background blobs as you scroll), but
  // it works by mutating `filter` on 4 large blurred elements on every
  // scroll tick, which forces the browser to fully re-rasterize each one —
  // measurably expensive, and directly tied to the exact gesture (scrolling
  // a chat) that most needs to feel smooth. Skipped on touch for the same
  // reason parallax and the ambient blob animation already are.
  function f33() {
    if (isCoarse) return;
